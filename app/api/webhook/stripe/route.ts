...
import { sendIntakeLinkEmail } from "@/lib/intakeLinkEmail";
...
export async function POST(req: Request) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (session?.id && session.payment_status === "paid") {
      const submission = await upsertPaidSubmissionFromStripeSession(session);
      await sendIntakeLinkEmail(submission);
    }
  }
